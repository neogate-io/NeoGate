...
use crate::{
    admin::{
        channel::{KeySelectionMode, ResponsesMode},
...
fn channel_candidate_from_row(row: &sqlx::postgres::PgRow) -> AppResult<ChannelCandidate> {
    ...
    Ok(ChannelCandidate {
        ...
        responses_mode: responses_mode_from_row(row)?,
...
fn responses_mode_from_row(row: &sqlx::postgres::PgRow) -> AppResult<ResponsesMode> {
    let mode: String = row.try_get("responses_mode")?;
    ResponsesMode::from_str(&mode)
}
