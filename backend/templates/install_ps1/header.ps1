...
$MsgEn = @{
  ...
  node_installing_pkg = 'Installing Node.js LTS system-wide with {0}...'
  node_winget_failed = 'winget could not complete Node.js installation (exit code {0}, HRESULT {1}), and node/npm are still unavailable.'
  node_choco_retry_prompt = 'Retry Node.js installation with the existing Chocolatey installation?'
...
$MsgZh = @{
  ...
  node_installing_pkg = '正在使用 {0} 安装 Node.js LTS（系统目录）...'
  node_winget_failed = 'winget 未能完成 Node.js 安装（退出码 {0}，HRESULT {1}），且当前仍无法使用 node/npm。'
  node_choco_retry_prompt = '是否使用现有的 Chocolatey 重试安装 Node.js？'
