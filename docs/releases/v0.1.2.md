...
- If you use background Responses image generation with NewAPI, keep `NEOGATE_ASSET_DIR` on persistent writable storage. Without it, assets use the platform temporary directory under `neogate/assets`.
...
- If you maintain custom Nginx files, copy the streaming timeout and buffering settings from `frontend/nginx/default.conf.template` or `deploy/nginx/source-build.conf.example`.
